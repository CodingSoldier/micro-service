...
public class GlobalExceptionHandlerControllerTest extends BaseTest {
  ...
  void testMicroServiceException() throws Exception {
    MockHttpServletRequestBuilder reqBuilder = MockMvcRequestBuilders.get(
        "/global/exception/handler/test/more/exception?id=1&name=MicroServiceException");
  ...
  void methodArgument() throws Exception {
    ...
    MockHttpServletRequestBuilder reqBuilder = MockMvcRequestBuilders.get(
        "/global/exception/handler/test/more/exception?id=sdfsd&name=名字");
    super.mockMvc.perform(reqBuilder)
        .andExpect(MockMvcResultMatchers.status().is(400))
        .andExpect(MockMvcResultMatchers.jsonPath("$.message",
    ...
    reqBuilder = MockMvcRequestBuilders.get(
        "/global/exception/handler/test/more/exception?id=1&name=IOException");
    super.mockMvc.perform(reqBuilder)
    ...
    reqBuilder = MockMvcRequestBuilders.get(
        "/global/exception/handler/test/more/exception?id=1&name=NullPointerException");
    super.mockMvc.perform(reqBuilder)
        .andExpect(MockMvcResultMatchers.status().is(500))
        .andExpect(MockMvcResultMatchers.jsonPath("$.message",
  ...
  void testException() throws Exception {
    MockHttpServletRequestBuilder reqBuilder = MockMvcRequestBuilders.get(
        "/global/exception/handler/test/more/exception?id=1&name=Exception");
    super.mockMvc.perform(reqBuilder)
        .andExpect(MockMvcResultMatchers.status().is(500))
        .andExpect(MockMvcResultMatchers.jsonPath("$.code",
