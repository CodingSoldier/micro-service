...
public class BasicControllerTest extends BaseTest {
  ...
  void paramsPath() throws Exception {
    MockHttpServletRequestBuilder reqBuilder = MockMvcRequestBuilders.get(
        "/basic/params/path/123?msg=信息&name=名字");
    super.mockMvc.perform(reqBuilder)
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.jsonPath("$.data",
            Matchers.equalTo("123信息名字")));

    MockHttpServletRequestBuilder reqBuilder2 = MockMvcRequestBuilders.get(
        "/basic/params/path/123?name=&msg=信息");
  ...
  void modelAttribute() throws Exception {
    MockHttpServletRequestBuilder reqBuilder = MockMvcRequestBuilders.get(
        "/basic/modelAttribute?age=123&name=名字");
    super.mockMvc.perform(reqBuilder)
        .andExpect(MockMvcResultMatchers.status().isOk())
  ...
  void body() throws Exception {
    MockHttpServletRequestBuilder reqBuilder = MockMvcRequestBuilders
        ...
        .content(
            "{\"phone\":\"18952145124\",\"name\":\"名字\",\"dateOfBirth\":\"2022-01-01 01:02:02\",\"id\":122,\"age\":11}");
    super.mockMvc.perform(reqBuilder)
        .andExpect(MockMvcResultMatchers.status().isOk())
