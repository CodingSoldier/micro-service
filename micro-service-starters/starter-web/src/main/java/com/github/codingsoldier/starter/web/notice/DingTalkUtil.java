...
public class DingTalkUtil {
  ...
  private static void send(String msg, Callback callback) throws Exception {
    ...
    mac.init(new SecretKeySpec(dingTalkProperties.getSecret().getBytes(StandardCharsets.UTF_8),
        "HmacSHA256"));
    //把字符串转化成字节形式
    ...
    String url =
        "https://oapi.dingtalk.com/robot/send?access_token=" + dingTalkProperties.getAccessToken()
    ...
    msg = "【" + dingTalkProperties.getTitle() + "】" + LocalDateTime.now().withNano(0).toString()
        .replace("T", " ") + " \n" + msg;

    textContent.put("content",
        StringUtils.substring(msg, 0, dingTalkProperties.getContentMaxLength()));
